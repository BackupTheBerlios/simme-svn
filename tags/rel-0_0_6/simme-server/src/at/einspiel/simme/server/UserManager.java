...
public class UserManager {
	...
	private final Map allUsers;
	private final Map managedGames;
			...
			synchronized (users) {
				if (users.size() > 1) {
					...
					for (Iterator i = users.iterator(); i.hasNext();) {
						...
						// add if user is in waiting state, and user's game is
						// not
						// managed yet
	...
	/**
	 * @param usersWaitingForGame
	 *            The usersWaitingForGame to set.
	 */
	...
	/**
	 * Sets the maximum time that idle users are allowed to not respond or
	 * update their status.
	 * 
	 * @param i
	 *            Time in seconds.
	 ...
	 * 
	 * @param i
	 *            Interval to update the managed users in milliseconds.
	 */
	...
	/**
	 * Returns the currently managed games as map.
	 * @return the currently managed games.
	 */
	Map getGames() {
		return managedGames;
	}
