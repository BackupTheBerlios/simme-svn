...
public class PrefForm extends List implements CommandListener {
   ...
   private Screen makePersPrefs() {
      ...
      final TextField tfWin = new TextField("Win-Msg", data[2], 20, TextField.ANY | TextField.NON_PREDICTIVE);
      ...
      if (data[5].equals("AT")) {
         cLoc = new ChoiceGroup("Location (opt.)", ChoiceGroup.POPUP, LOCATION, null);
      } else {
      ...
      final TextField tfPlatform =
         new TextField("Client Model", data[6], data[6].length(), TextField.ANY | TextField.UNEDITABLE);
