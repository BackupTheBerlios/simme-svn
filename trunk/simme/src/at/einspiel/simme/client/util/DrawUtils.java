//----------------------------------------------------------------------------
//[Simme]
//    Java Source File: DrawUtils.java
//               $Date: 2004/09/07 13:27:38 $
//           $Revision: 1.2 $
//----------------------------------------------------------------------------
...
public class DrawUtils {
	 ...
	 * 
	 * @param g
	 *            Graphics context on which to draw.
	 * @param xStart
	 ...
	 *            End y coordinate.
	 * @param thickness
	 *            Thickness of the line. Should be greater than 1
	 */
	public static void drawLine(Graphics g, int xStart, int yStart, int xEnd, int yEnd,
		...
		for (int i = 0; i < thickness; i++) {
			...
			//System.out.println("g.drawLine(" + x1 + "," + y1 + "," + x2 + ","
			// + y2 + ");");
			// draw single line
			if (notSteep) {
	 ...
	 *            End y coordinate.
	 * @param thickness
	 *            Thickness of the line. Should be greater than 2
	 * @param innerColor
	 *            Color for inner part of line.
	 * @param outerColor
	 *            Color for line border.
	 * @param dotted
	 *            whether to draw the line dotted.
	 */
	public static void drawLineWithBorder(Graphics g, int xStart, int yStart, int xEnd,
			int yEnd, int thickness, int innerColor, int outerColor, boolean dotted) {
		if (thickness < 2) {
			return;
		}
		...
		/*
		 * if (drawAxisLine(g, x1, y1, x2, y2, thickness)) { int pos;
		 * 
		 * if (x1 == x2) { g.setColor(outerColor); pos = x1 + r; g.drawLine(pos,
		 * y1, pos, y2); pos = x1 - r; g.drawLine(pos, y1, pos, y2); } else if
		 * (y1 == y1) { g.setColor(outerColor); pos = y1 + r; g.drawLine(x1,
		 * pos, x2, pos); pos = y1 - r; g.drawLine(x1, pos, x2, pos); }
		 * 
		 * return; }
		 */
	 ...
	 *            Graphics context on which to draw.
	 * @param x
	 *            the <i>x </i> coordinate of the upper-left corner of the arc
	 *            to be filled.
	 * @param y
	 *            the <i>y </i> coordinate of the upper-left corner of the arc
	 *            to be filled.
	 * @param width
	 ...
	 */
	public static void fillCircleWithBorder(Graphics g, int x, int y, int width, int height,
	 ...
	 */
	public static void fillArcWithBorder(Graphics g, int x, int y, int width, int height,
			int startAngle, int arcAngle, int innerColor, int outerColor) {
		g.setColor(outerColor);
	 ...
	 *         otherwise.
	 */
	private static boolean drawAxisLine(Graphics g, int x1, int y1, int x2, int y2,
			int thickness) {
		int l;
