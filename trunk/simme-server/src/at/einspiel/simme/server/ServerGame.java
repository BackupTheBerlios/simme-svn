...
public class ServerGame implements IGame {
	...
	private static final SimpleDateFormat FORMAT_ID = new SimpleDateFormat("yyyyMMdd-HHmmss");
	 ...
	 *             if rules are broken (e.g. same players).
	 */
	public ServerGame(User p1, User p2) throws IllegalArgumentException {
		...
		game.setId(date + "---" + player1.getNick() + "---" + player2.getNick());
	...
	/**
	 * Sets the players for this game. Additionally the game information is
	 * updated.
	 * @param p1
	 *            player 1.
	 * @param p2
	...
	public void setPlayers(User p1, User p2) {
		...
		// update game information, if possible
		GameInfo info = game.getGameInfo();
		info.setP1Name(p1.getNick());
		info.setP1Info(p1.getInfo());
		info.setP2Name(p2.getNick());
		info.setP2Info(p2.getInfo());
	}

	/**
	 * Starts the game. Prepares everything for the game start and starts the
	 * timer.
	 */
	...
	/**
	 * Returns an id that is easier to read, than the real id.
	 * @return an easy to read id, that can be presented to the user.
	 * @see #getId()
	 */
	public String getNiceId() {
		return FORMAT_ID.format(getDate());
	}

	/** @see at.einspiel.base.IGame#getGame() */
	public Game getGame() {
		return game;
	}
	 ...
	 * 
	 * @param u
	 *            the user who performs the move.
	 * @param edge
	 *            the edge to select (between 0 and 14).
